...
use colored::*;
use crypto::{generate_private_key, pow_mod, Lcg, G, P};
...
fn main() {
    ...
    match args.command {
        Commands::Server { port } => {
            ...
            println!(
                "{}",
                format!("[SERVER] Listening on {}", address).blue().bold()
            );
            ...
            if let Ok((stream, addr)) = listener.accept() {
                println!(
                    "{}",
                    format!("[CONNECTION] Client connected from {}", addr)
                        .green()
                        .bold()
                );
        ...
        Commands::Client { address } => {
            println!(
                "{}",
                format!("[CLIENT] Connecting to {}...", address).blue()
            );
